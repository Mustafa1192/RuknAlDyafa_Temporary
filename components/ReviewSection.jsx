...
//   if (days >= 7 && days < 30) return `${Math.floor(days / 7)} week${Math.floor(days / 7) > 1 ? 's' : ''} ago`;
//   if (days >= 30 && days < 365) return `${months} month${months > 1 ? 's' : ''} ago`;
//   if (days >= 365) return `${years} year${years > 1 ? 's' : ''} ago`;
...
//       <div className="max-w-7xl mx-auto">
//         <h2 className="text-2xl font-bold text-center mb-10">
//           {language === "english" ? "Client Stories of Delicious Moments" : "قصص العملاء من اللحظات الشهية"}
//         </h2>
...
//   useEffect(() => {
//     const intervalId = setInterval(() => {
//       setElapsedTime(formatTime(review.time));
//     }, 60000);
...
//   return (
//     <div className="bg-gray-100 shadow-lg rounded-lg p-6 border border-gray-300 min-w-[354px] sm:min-w-[300px] lg:w-[300px]">
//       <div className="flex items-center mb-3">
//         {Array.from({ length: review.rating }).map((_, i) => (
...
//           </svg>
//         ))}
//         <FcGoogle className="w-6 h-6 ml-auto" />
//       </div>

//       <p className="text-sm text-gray-700 mb-4">
//         {isExpanded ? review.text : `${review.text.substring(0, 98)}...`}
//         <button onClick={() => setIsExpanded(!isExpanded)} className="text-blue-600 font-medium hover:underline ml-2">
//           {isExpanded ? "Show less" : "Show more"}
//         </button>
//       </p>

//       <div className="flex items-center">
//         <a href="https://www.google.com/maps/place/Rukn+Al+Dyafa+%7C+Hospitality+Service/" target="_blank" rel="noopener noreferrer" className="flex items-center">
//           <img src={review.profileImage} alt={review.name} className="w-8 h-8 rounded-full mr-3" />
//           <div>
//             <p className="text-sm font-semibold text-gray-800">{review.name}</p>
